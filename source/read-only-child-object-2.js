...
function fromDto( dto ) {
  ...
  if (extensions.fromDto)
    extensions.fromDto.call( self, getTransferContext.call( this, false ), dto );
  else
  ...
  return new Promise( ( fulfill, reject ) => {
    if (method === M_FETCH ?
      ...
      (extensions.dataFetch ?
        // *** Custom fetch.
        extensions.$runMethod( 'fetch', self, getDataContext.call( self ), data, method ) :
        // *** Standard fetch.
        new Promise( ( f, r ) => {
...
class ReadOnlyChildObject extends ModelBase {
  ...
  checkRules() {
    ...
    const store = _store.get( this );
    const context = new ValidationContext.call( this, store, brokenRules );
...
class ReadOnlyChildObjectFactory {
  ...
  constructor( name, properties, rules, extensions ) {
    var check = Argument.inConstructor( ModelType.ReadOnlyChildObject );
    ...
    properties.verifyChildTypes( [ ModelType.ReadOnlyChildCollection, ModelType.ReadOnlyChildObject ] );
    ...
    Model.load = function ( parent, data, eventHandlers ) {
      var instance = new Model( parent, eventHandlers );
