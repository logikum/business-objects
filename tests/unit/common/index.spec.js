...
describe( 'Shared component index', () => {
  ...
  it( 'properties return correct components', () => {
    ...
    expect( common.ModelType ).toBe( ModelType );
    expect( new common.ModelBase() )
      .toEqual( jasmine.any( ModelBase ) );
    expect( new common.CollectionBase() )
      .toEqual( jasmine.any( CollectionBase ) );

    expect( new common.ExtensionManager( 'data_source', '/model/path' ) )
      .toEqual( jasmine.any( ExtensionManager ) );
    expect( new common.EventHandlerList() )
      .toEqual( jasmine.any( EventHandlerList ) );
    expect( new common.DataStore() )
      .toEqual( jasmine.any( DataStore ) );
    expect( common.ModelState ).toEqual( jasmine.any( Enumeration ) );
    expect( new common.ModelError( 'message' ) )
      .toEqual( jasmine.any( ModelError ) );

    expect( new common.PropertyInfo( 'property', text ) )
      .toEqual( jasmine.any( PropertyInfo ) );
    expect( common.PropertyFlag ).toBe( PropertyFlag );
    expect( new common.PropertyManager() )
      .toEqual( jasmine.any( PropertyManager ) );
    expect( new common.PropertyContext( 'model', [], getValue, setValue ) )
      .toEqual( jasmine.any( PropertyContext ) );
    expect( new common.ClientTransferContext( [], getValue, setValue ) )
      .toEqual( jasmine.any( ClientTransferContext ) );
    expect( new common.DataTransferContext( [], getValue, setValue ) )
      .toEqual( jasmine.any( DataTransferContext ) );

    expect( common.DataPortalAction ).toBe( DataPortalAction );
    expect( new common.DataPortalContext( dao, [], getValue, setValue ) )
      .toEqual( jasmine.any( DataPortalContext ) );
    expect( common.DataPortalEvent ).toBe( DataPortalEvent );
    expect( new common.DataPortalEventArgs( DataPortalEvent.preCreate, 'model' ) )
      .toEqual( jasmine.any( DataPortalEventArgs ) );
    expect( new common.DataPortalError( 'type', 'name', 0, {} ) )
      .toEqual( jasmine.any( DataPortalError ) );
  } );
} );
